...
# Read the dataset
EIB_35s <- read_excel('C:/Users/Asia/Desktop/EIB_FPN_CAP_behavior_AF/FPN_CAP_behavior_36s.xlsx')
fMRI <- EIB_35s
...
hist(fMRI$INDegree, main = "Histogram of INDegree", xlab = "INDegree", col = "lightblue", breaks = 20)
qqnorm(fMRI$INDegree, main = "Q-Q Plot of INDegree"); qqline(fMRI$INDegree, col = "red")

hist(fMRI$OUTDegree, main = "Histogram of OUTDegree", xlab = "OUTDegree", col = "lightblue", breaks = 20)
qqnorm(fMRI$OUTDegree, main = "Q-Q Plot of OUTDegree"); qqline(fMRI$OUTDegree, col = "red")

hist(fMRI$Resilience, main = "Histogram of Resilience", xlab = "Resilience", col = "lightblue", breaks = 20)
qqnorm(fMRI$Resilience, main = "Q-Q Plot of Resilience"); qqline(fMRI$Resilience, col = "red")

hist(fMRI$BtwCentrality, main = "Histogram of BtwCentrality", xlab = "BtwCentrality", col = "lightblue", breaks = 20)
qqnorm(fMRI$BtwCentrality, main = "Q-Q Plot of BtwCentrality"); qqline(fMRI$BtwCentrality, col = "red")

hist(fMRI$Occurrences, main = "Histogram of Occurrences", xlab = "Occurrences", col = "lightblue", breaks = 20)
qqnorm(fMRI$Occurrences, main = "Q-Q Plot of Occurrences"); qqline(fMRI$Occurrences, col = "red")

# Perform Shapiro-Wilk test for normality
shapiro_test_INDegree <- shapiro.test(fMRI$INDegree)
shapiro_test_OUTDegree <- shapiro.test(fMRI$OUTDegree)
shapiro_test_Resilience <- shapiro.test(fMRI$Resilience)
shapiro_test_BtwCentrality <- shapiro.test(fMRI$BtwCentrality)
shapiro_test_Occurrences <- shapiro.test(fMRI$Occurrences)
...
for (column_name in columns_to_substitute) {
  ...
  fit_Acc <- glmer(formula_Acc, data = fMRI, family = Gamma(link = "log"), control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
  
  # Model for Reaction Time (RT)
  formula_RT <- as.formula(paste("RT ~", column_name, "* Runs + Gender + Age + (1|Subject)"))
  fit_RT <- glmer(formula_RT, data = fMRI, family = Gamma(link = "log"), control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
  
  # Model for d-prime (dprime)
  formula_dprime <- as.formula(paste("dprime ~", column_name, "* Runs + Gender + Age + (1|Subject)"))
  fit_dprime <- glmer(formula_dprime, data = fMRI, family = Gamma(link = "log"), control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
